...
import com.sos.recipe.model.vo.AttachmentRecipe;
...
//				String[] stepFiles = multiRequest.get
				...
				AttachmentRecipe at = null;
				for (int i = 0; i < stepContent.length; i++) {
					if(multiRequest.getOriginalFileName("stepUpfile") != null) {
			            at = new AttachmentRecipe();
			            at.setFileName(multiRequest.getOriginalFileName("stepUpfile"));
			            at.setFileChangeName(multiRequest.getFilesystemName("stepUpfile"));
			            at.setFileRoute("resouces/uploadFiles/");
			            
			            //기존 파일이 있을 때 ~인데 stepAttachUrl 이걸로 비교하는거 맞나요? originFileNo이거로 비교해야하나요?
			            if(multiRequest.getParameter("stepAttachUrl") != null) {
			               at.setProFileNo(Integer.parseInt(multiRequest.getParameter("stepAttachUrl")));
			            }else {
			               at.setRefNo(recipeNo);
			            }
			         }
				}
				...
				for (int i = 0; i < stepContent.length; i++) {
					if(multiRequest.getOriginalFileName("stepUpfile") != null) {
			            ...
			            if(multiRequest.getParameter("originFileNo") != null) {
			               at.setProFileNo(Integer.parseInt(multiRequest.getParameter("originFileNo")));
			            }else {
			               at.setRefNo(recipeNo);
			            }
			         }
