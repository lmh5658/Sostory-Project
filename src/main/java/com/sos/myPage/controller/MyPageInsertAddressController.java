...
public class MyPageInsertAddressController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 
		 * case 01) 로그인된 사용자가 정상적인 경로(마이페이지) 배송지 등록요청을 할경우
		 *          전달받은 데이터로 배송지 등록진행 
		 * 
		 * case 02) 미로그인 사용자가 비정상적인 경로(URL 공격 등)을 통해 배송지 등록요청을 할경우 
		 *          응답페이지 : 로그인페이지
		 *          응답메세지 : "로그인을 먼저진행해주세요."
		 */
		...
		// 배송지 추가등록 요청처리 결과에 따른 응답화면 및 알림메세지
		if(result > 0) {
