...
/**
 * Servlet implementation class ProductNewSearchdlist
 */
@WebServlet("/sdlistNew.pr")
public class ProductNewSearchdlistController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ProductNewSearchdlistController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(list.isEmpty()) {
			request.getRequestDispatcher("/views/newProduct/productNewNotdlist.jsp").forward(request, response);
		}else {			
			request.getRequestDispatcher("/views/newProduct/producNewdlist.jsp").forward(request, response);			
		}
