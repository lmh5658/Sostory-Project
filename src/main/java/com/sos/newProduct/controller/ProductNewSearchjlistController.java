...
/**
 * Servlet implementation class ProductNewSearchjlistController
 */
@WebServlet("/sjlistNew.pr")
public class ProductNewSearchjlistController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ProductNewSearchjlistController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(list.isEmpty()) {
			request.getRequestDispatcher("/views/newProduct/productNewNotjlist.jsp").forward(request, response);
		}else {
			request.getRequestDispatcher("/views/newProduct/productNewjlist.jsp").forward(request, response);			
		}
